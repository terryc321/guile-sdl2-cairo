/*
  ...
  library version for guile of SDL2 
...
(three) => 3
sdl-init
(sdl-init) => should see a window open up

ACTUALLY NO - FAILS , cannot determine SDL_Init , likely SDL_Init is a macro
This is an example of limitation of C language , macro is eliminated from source and symbol simply vanishes
without trace

scheme@(guile-user)> (sdl-init)
