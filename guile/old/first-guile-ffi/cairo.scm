...
(define (cairo)  
  (let ((font-size 12)
    ...
    ;;                                                                    sdl_surface->h,
    ;;                                                                    sdl_surface->pitch);    
    (define cairo-surface (cairo-image-surface-create-for-data (surface-pixels tmp-surface)
							       *cairo-format-rgb24*
							       (surface-width tmp-surface)
							       (surface-height tmp-surface)
							       (surface-pitch tmp-surface)))

    ;; helpful abbreviation - something like cairo-render (cr)
    ...
    (define optimized-surface
      (let ((screen-format (surface-pixelformat surface)))
    ...
    (while (not quit)
	   ...
	   ;; cairo_fill(cr);
	   ;;(format #t "some cairo stuff should happend shortly~%")
	   (cairo-set-source-rgba cr 0.5 1.0 1.0 1.0)
	   (cairo-rectangle cr 200.0 200.0 50.0 100.0)
	   (cairo-fill cr)

	   ;; SDL_Texture *texture = SDL_CreateTextureFromSurface(render, sdl_surface);
	   ;; SDL_FreeSurface(sdl_surface);
	   (when (not (equal? cairo-texture %null-pointer))
	     (sdl-destroy-texture cairo-texture))
	   
	   (set! cairo-texture (sdl-create-texture-from-surface render tmp-surface))
	   ;; free - what surface ??
